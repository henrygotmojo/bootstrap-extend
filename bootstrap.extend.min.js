$(function(){$(document).ajaxError(function(evt,jqXHR,ajaxSettings,thrownError){if(!$('body[data-ajax-error]').length||!$('body').attr('data-ajax-error').length){$('body').attr('data-ajax-error','modal')}
if($('body').attr('data-ajax-error')=='modal'&&$('body.modal-open').length){var $modalVisible=$('.modal.show');if(!$('#bsx-error-alert').length){$('<div id="bsx-error-alert" class="alert alert-danger" role="alert"></div>').prependTo($modalVisible.find('.modal-body')).on('click',function(){$(this).slideUp()}).hide()}
$('#bsx-error-alert').html('').append('<h3 class="mt-0 text-danger">Error</h3>').append('<pre>'+jqXHR.responseText+'</pre>').append('<small><em class="text-danger">'+ajaxSettings.url+'</em></small>').filter(':visible').hide().fadeIn().end().filter(':hidden').slideDown();$modalVisible.animate({scrollTop:0})}else if($('body').attr('data-ajax-error')=='modal'){if(!$('#bsx-error-modal').length){$('body').append(`
			...
			`)}
$('#bsx-error-modal').modal('show').find('.modal-body').html('').append('<h3 class="mt-0 text-white">Error</h3>').append('<pre>'+jqXHR.responseText+'</pre>').append('<small><em class="text-warning">'+ajaxSettings.url+'</em></small>')}else if($('body').attr('data-ajax-error')=='alert'){alert('[Error]\n'+jqXHR.responseText+'\n\n'+ajaxSettings.url)}else{console.log('[Error] '+jqXHR.responseText+' ('+ajaxSettings.url+')')}});$(document).on('click','[href][data-target][data-toggle=ajax-modal],[data-href][data-target][data-toggle=ajax-modal]',function(evt){evt.preventDefault();var $btn=$(this);var $modal=$($btn.attr('data-target'));if(!$modal.length){console.log('[ERROR] Target modal not found ('+$btn.attr('data-target')+')');return!1}else if(!$modal.find('.modal-dialog').length){console.log('[ERROR] Target modal has no <DIV.modal-dialog> element ('+$btn.attr('data-target')+')')}
if(!$modal.find('.modal-content').length){$modal.find('.modal-dialog').append('<div class="modal-content"></div>')}
